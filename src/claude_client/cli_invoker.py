        ...
        def extract_from_string(s: str) -> Optional[Dict[str, Any]]:
            ...
            while True:
                ...
                if start == -1:
                    break
                try:
                    # ⚡ Bolt Optimization: Use raw_decode(s, start) instead of raw_decode(s[start:]).
                    # This avoids O(N^2) memory copying from string slicing on large outputs.
                    # Note: raw_decode with index returns the absolute index in the original string.
        ...
        try:
            ...
            while True:
                ...
                if start == -1:
                    break
                try:
                    # ⚡ Bolt Optimization: Use raw_decode(s, start) to avoid O(N^2) string slicing memory copying.
